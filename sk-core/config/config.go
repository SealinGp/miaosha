...
import (
	...
	_ "miaosha/pkg/bootstrap"
	conf "miaosha/pkg/config"
	"miaosha/sk-core/service/srv_product"
	"miaosha/sk-core/service/srv_user"
	"os"
	"sync"
)
...
func init() {
	...
	if err := conf.SubParse("redis", &conf.Redis); err != nil {
		Logger.Log("Fail to parse trace", err)
	}

	if err := conf.SubParse("service", &conf.SecKill); err != nil {
		Logger.Log("Fail to parse trace", err)
	}
